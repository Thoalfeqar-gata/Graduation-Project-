...
'''
def run_face_recognition(face_detector = 'hog', frame_size = (640, 480), apply_clahe = True, model_path = 'data/models/MobileNetV2 128 optimized/model.tflite', model_type = 'tflite', face_size = (100, 100)): 
    ...
    while True:
        ...
        if face_detector == 'hog':
            ...
            locations = opencv_face_detector.detectMultiScale(frame, 1.1, 10)
        ...
        for x, y, w, h in locations:
            ...
            if model_type == 'tflite':
                ...
                model.invoke()
                prediction = int(np.argmax(model.get_tensor(output_details[0]['index']), -1))
            elif model_type == 'tf':
                prediction = int(np.argmax(model.predict(input_image), -1))
...
model_path = 'data/models/MobileNetV2 192'
run_face_recognition(apply_clahe = False, face_detector = 'opencv', model_path = model_path, model_type = 'tf')
